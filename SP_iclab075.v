...
integer i;
//////////////////clkgated
reg ctrl_gray,ctrl_addsub,ctrl_sma,ctrl_sort;

wire G_clk_gray;
wire G_sleep_gray = ctrl_gray;
GATED_OR GATED_gray (
	.CLOCK( clk ),
	.SLEEP_CTRL( cg_en && G_sleep_gray ),	// gated clock
	.RST_N( rst_n ),
	.CLOCK_GATED( G_clk_gray )
);
always @(*) 
begin
	if( ns==S_GRAY &&  mode[0] == 1) ctrl_gray=0;
	else ctrl_gray=1;
end

wire G_clk_addsub;
wire G_sleep_addsub = ctrl_addsub;
GATED_OR GATED_addsub (
	.CLOCK( clk ),
	.SLEEP_CTRL( cg_en && G_sleep_addsub ),	// gated clock
	.RST_N( rst_n ),
	.CLOCK_GATED( G_clk_addsub )
);
always @(*) 
begin
	if( ns==S_ADDSUB && mode[1] == 1) ctrl_addsub=0;
	else ctrl_addsub=1;
end

wire G_clk_sma;
wire G_sleep_sma = ctrl_sma;
GATED_OR GATED_sma (
	.CLOCK( clk ),
	.SLEEP_CTRL( cg_en && G_sleep_sma ),	// gated clock
	.RST_N( rst_n ),
	.CLOCK_GATED( G_clk_sma )
);
always @(*) 
begin
	if( ns == S_SMA && mode[2] == 1) ctrl_sma=0;
	else ctrl_sma=1;
end
/*
wire G_clk_sort;
wire G_sleep_sort = ctrl_sort;
GATED_OR GATED_sort (
	.CLOCK( clk ),
	.SLEEP_CTRL( cg_en && G_sleep_sort ),	// gated clock
	.RST_N( rst_n ),
	.CLOCK_GATED( G_clk_sort )
);
always @(*) 
begin
	if(ns == S_SORT ) ctrl_sort=0;
	else ctrl_sort=1;
end
*/
/////////////////////////////////
...
GRAY sub0(.clk(G_clk_gray) ,.in_valid(in_valid_gray), .rst_n(rst_n), 
		.a0(gray_in[0]), .a1(gray_in[1]) , .a2(gray_in[2]), .a3(gray_in[3]), .a4(gray_in[4]), .a5(gray_in[5]), .a6(gray_in[6]), .a7(gray_in[7]), .a8(gray_in[8])
		,.out_valid(out_valid_gray) ,.out0(gray_out[0]),.out1(gray_out[1]),.out2(gray_out[2]),.out3(gray_out[3]),.out4(gray_out[4])
		,.out5(gray_out[5]),.out6(gray_out[6]),.out7(gray_out[7]),.out8(gray_out[8]));
genvar gen_i;
generate
...
ADDorSUB sub1(.clk(G_clk_addsub) ,.in_valid(in_valid_addsub), .rst_n(rst_n), .a0(addsub_in[0]), .a1(addsub_in[1]) , .a2(addsub_in[2]), .a3(addsub_in[3]), .a4(addsub_in[4]), .a5(addsub_in[5]), .a6(addsub_in[6]), .a7(addsub_in[7]), .a8(addsub_in[8])
...
SMA sub2(.clk(G_clk_sma) ,.in_valid(in_valid_sma), .rst_n(rst_n), .a0(sma_in[0]), .a1(sma_in[1]) , .a2(sma_in[2]), .a3(sma_in[3]), .a4(sma_in[4]), .a5(sma_in[5]), .a6(sma_in[6]), .a7(sma_in[7]), .a8(sma_in[8])
...
always@(posedge clk or negedge rst_n) begin
	...
	else if(ns == PROCESS || ns == OUT )begin
